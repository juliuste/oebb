...
const createServices = (c) => ({
	...
	price: Math.round(c.price.price) + " EUR",
	seat_available: 999
})
...
const createTrains = (s) => ({
	...
	departure_at: s.from.departure.replace(".000",""),
	arrival_at: s.to.arrival.replace(".000",""),
...
const createSection = (c) => ({
	carrier_code: (c.connection.sections[0].category.name === 'ICE') ? ('DBICE') : ('OEBB' + c.connection.sections[0].category.name),
