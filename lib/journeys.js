...
const createOffers = (p) => ({
	code: (p && isNumber(p.price)&&!p.specialNote) ? "Flex":"NonFlex",
	description: (p && isNumber(p.price)&&!p.specialNote) ? "Flex Ticket":p.specialNote.en
})
