...
const journeys = async (origin, destination, opt = {}) => {
	...
	const { travelActions } = await post('https://shop.oebbtickets.at/api/offer/v2/travelActions', {
	...
	fetchJourneys: do {
		...
		const endpoint = lastConnectionId ? 'https://shop.oebbtickets.at/api/hafas/v1/timetableScroll' : 'https://shop.oebbtickets.at/api/hafas/v4/timetable'
		...
		const { offers = [] } = (options.prices && rawJourneyIds.length > 0) ? await get('https://shop.oebbtickets.at/api/offer/v1/prices', {
