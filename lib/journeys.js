...
const createSection = (c) => ({
	carrierCode: 'OBB' + c.connection.sections[0].category.name,
	carrierName: 'ÖBB ' + c.connection.sections[0].category.longName.en,
...
const createJourney = (c) => ({
	...
	destination: c.connection.sections[c.connection.sections.length-1].to.name,
	destinationStationCode: c.connection.sections[c.connection.sections.length-1].to.esn, 
	numberOfInterchange: c.connection.switches,
	travelDuration: Math.floor(c.connection.duration/3600000).toString() + ":" + Math.floor(((c.connection.duration/1000)%3600)/60).toString(),
