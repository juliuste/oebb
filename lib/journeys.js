...
const journeys = async (origin, destination, date = new Date(), adult, children, opt = {}) => {
	...
	const buildPassengers = function (numberOfAdult, numberOfChild) {
		var passengers = []
		for (let index = 0; index < numbnerOfAdult; index++) {
	...
	const connections = await h.post('https://tickets.oebb.at/api/hafas/v4/timetable', {
		...
		'passengers': buildPassengers(numberOfAdult, numberOfChild),
		'count': 5, // must be <= 5
		'debugFilter': {
